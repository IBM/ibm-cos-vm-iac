...
GW=${BASE_IP%.*}.1
sudo ./cos-manager.expect COS${SYSTEM}-Manager1 ${BASE_IP}0 ${GW} ${DNS_IPS} ${NTP_IPS}
sudo ./cos-accesser.expect COS${SYSTEM}-Accesser1 ${BASE_IP}1 ${GW} ${DNS_IPS} ${BASE_IP}0
for i in `seq 1 ${NUM_SLICESTORS}`; do
  sudo ./cos-slicestor.expect COS${SYSTEM}-Slicestor${i} ${BASE_IP}$(($i + 1)) ${i} ${GW} ${DNS_IPS} ${BASE_IP}0
done
